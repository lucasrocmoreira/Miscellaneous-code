...
qsub -l h_vmem=32g -l h_rt=20:00:00 -b y -p -10 -N rtc.$currID -cwd -o $outDir/$currID.6.indel_targets.out -j y -V -hold_jid ind.$currID java -Xmx4g -jar $gatkVersion/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $refFile -I $outDir/$currID.5.mark_dup.bam -o $outDir/$currID.6.indel_targets.intervals --tmp-dir $tempDir
qsub -l h_vmem=32g -l h_rt=20:00:00 -b y -p -10 -N idr.$currID -cwd -o $outDir/$currID.6.indel_realigned.out -j y -V -hold_jid rtc.$currID java -Xmx4g -jar $gatkVersion/GenomeAnalysisTK.jar -T IndelRealigner -R $refFile -I $outDir/$currID.5.mark_dup.bam -targetIntervals $outDir/$currID.6.indel_targets.intervals -o $outDir/$currID.6.indel_realigned.bam --tmp-dir $tempDir
